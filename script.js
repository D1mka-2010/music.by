    ...
    newMusicItem.innerHTML = `
        ...
        <div class="menu">
            ...
            <div class="menu-options">
                <button class="like-button">❤️ Лайк</button>
                <button class="dislike-button">👎 Дизлайк</button>
                <button class="download-button">⬇️ Скачать</button>
            </div>
        </div>
    `;
...
document.addEventListener('click', (e) => {
    if (e.target.classList.contains('like-button')) {
        ...
        // Добавляем трек в плейлист "Любимые треки"
        const favoritePlaylist = document.getElementById('favorite-playlist');
        const playlistTracks = favoritePlaylist.querySelector('.playlist-tracks');

        // Проверяем, не добавлен ли трек уже
        const isAlreadyAdded = Array.from(playlistTracks.children).some(item => item.getAttribute('data-title') === title);

        if (!isAlreadyAdded) {
            const newTrack = document.createElement('div');
            newTrack.className = 'music-item';
            newTrack.setAttribute('data-title', title);
            newTrack.setAttribute('data-author', author);
            newTrack.setAttribute('data-file', file);
            newTrack.innerHTML = `
                <img src="cover1.jpg" alt="Обложка" class="music-cover">
                <div class="music-info">
                    <h3>${title}</h3>
            ...
            `;
            playlistTracks.appendChild(newTrack);

            // Обновляем количество треков в плейлисте
            const trackCount = playlistTracks.children.length;
            favoritePlaylist.querySelector('.track-count').textContent = trackCount;
        }
        ...
        if (track) {
            track.likes += 1;
            saveTracks(tracks);
            musicItem.querySelector('.music-info p').textContent = `Лайки: ${track.likes} | Дизлайки: ${track.dislikes} | Скачивания: ${track.downloads}`;
        }
    }
